...
export function OfflineQueueStatus() {
  ...
  const syncQueue = async () => {
    ...
    if (successCount === stored.length) {
      ...
      setFeedback(`Synced ${successCount} offline events`);
    } else {
      const remaining = stored.slice(successCount);
      localStorage.setItem(OFFLINE_QUEUE_KEY, JSON.stringify(remaining));
      setQueue(remaining);
      setFeedback(`Synced ${successCount}/${stored.length} events`);
    }
    setSyncing(false);
      ...
      <CardContent className="p-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {queue.length > 0 && (
              <Badge variant="secondary" className="bg-amber-100 text-amber-700 text-xs">{queue.length}</Badge>
            )}
          </div>
          <div className="flex items-center gap-2">
        ...
        {feedback && (
          <p className="text-xs text-emerald-600 mt-2">{feedback}</p>
        )}
      </CardContent>
    </Card>
  );
